...
function [SMstate,SMVstate,KFstate,KFVstate,KFresid,MLE]=KalmanFSbreaks(Y,X,beta,SigmaBB, R, tvp_period_idx)
...
Sw=SigmaBB{1};             % variance of innovation in the state (i.e. coefs in the Taylor rule)
Sv=R{1};                   % variance of policy shocks (i.e. error terms in the Taylor rule)
...
Sz0=SigmaBB{1};            % initial uncertainty about the state (i.e., coefs in the Taylor rule)
...
for i1=1:smplT
    ...
    i1_idx = sum(i1 >= tvp_period_idx(1:end-1));
    Sv = R{i1_idx};
    Sw = SigmaBB{i1_idx};
