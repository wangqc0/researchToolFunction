function [beta_0_response, beta_0_response_lower, beta_0_response_upper, m_1_aic, m_2_aic, q_aic, lp_aic] = ...
    lp_irf_jorda_bs_2_shock(Y, x, ind_diff, h_max, irf_band_confidence, min_delay, m_1_max, m_2_max, q_max, n_bs)
    arguments
        Y (:, :) double % responses
        x (:, 2) double % shock
    ...
    for i_response = 1:n_response
        ...
        for h = min_delay:(h_max + min_delay)
            if any(i_response == ind_diff)
                [beta, ~, beta_draw] = localProjDiffTwoShock_bs(y, x, yy, h, m_1_min, m_1_aic, m_2_min, m_2_aic, q_min, q_aic, n_bs);
            else
                [beta, ~, beta_draw] = localProjTwoShock_bs(y, x, yy, h, m_1_min, m_1_aic, m_2_min, m_2_aic, q_min, q_aic, n_bs);
            end
            beta_0_1 = beta(2);
